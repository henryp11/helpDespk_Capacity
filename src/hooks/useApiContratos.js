...
const API = `${process.env.NEXT_PUBLIC_API_ROUTE}/api/v1/contratos`;

const useApiContratos = () => {
  ...
  //para usar la API que detecte el id_contrato, al usar el método get de la api se debe
  //Cambiar la URL inicial mandando el id_contrato como queryParam de la ruta, por eso al iniciar la función se arma la URL con la API, añadiendo el id_contrato
  //De está forma el API utilizará el método GET especifico para obtener solo el dato del registro a buscar
  const getContratoById = async (id_contrato) => {
    const API_PARAMS = `${API}/${id_contrato}`;
    //Obtengo las variables del LocalStorage
    const tokenLS = localStorage.getItem('jwt');
    //Si existen las variables, se las transforma en JSON para su uso
    const tokenStorage = tokenLS && JSON.parse(tokenLS);
  ...
  const updateContrato = async (id_contrato, dataUpdate) => {
    const API_PARAMS = `${API}/${id_contrato}`;
    //Obtengo las variables del LocalStorage
    const tokenLS = localStorage.getItem('jwt');
    //Si existen las variables, se las transforma en JSON para su uso
  ...
  //Eliminar Contrato
  const deleteContrato = async (id_contrato, message) => {
    const API_PARAMS = `${API}/${id_contrato}`;
    //Obtengo las variables del LocalStorage
    const tokenLS = localStorage.getItem('jwt');
    //Si existen las variables, se las transforma en JSON para su uso
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  toast.dismiss(t.id);
                  response && getContratos(); //Una vez eliminado realizo "hotReload" para refrescar datos
                  toast.success(response.data.message, {
                    style: {
...
export default useApiContratos;
